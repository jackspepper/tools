...
`protocol3Parser` is an R package for importing and tidying ProtoCOL 3 reports exported as `.ods` spreadsheets: colony counts, antibiotic resistance (zone diameter), and inhibition zone reports.

The package parses each plate section from a ProtoCOL 3 report into a structured object containing:

- Plate metadata
- Colony count data, antibiotic zone/susceptibility data, or inhibition zone data
...
# Inhibition zone report example
inhibition_file <- system.file(
  "extdata",
...
For audit compliance, `parse_protocol3()`, `parse_protocol3_abx()`, and `parse_protocol3_inhibition()` all automatically record provenance metadata by default (`include_version = TRUE`):
...
## Inhibition Zone Reports

For ProtoCOL 3 inhibition zone plate reports, use `parse_protocol3_inhibition()`. It has the same interface as `parse_protocol3_abx()`, but reads the inhibition-report columns (`Sector`, `Zone Name`, `Zone Diameter (mm)`, `Flags`, `Zone Area (mm^2)`) — note there's no `Antibiotic Susceptibility` column, and both `Zone Diameter (mm)` and `Zone Area (mm^2)` are converted to numeric:
...
ProtoCOL 3 stores multiple flags as a single space-separated string (e.g. `"E M"`). All three parsers add a `Flags_list` column by default (`split_flags = TRUE`), giving each row a character vector of individual flags:
...
If `exclude` is not supplied, all three functions return the plain per-plate list as before (no `$included`/`$excluded` wrapper) — this is fully backward compatible with existing code.

## Package Structure & Functions

### R Source Files

- `R/parse_protocol3.R`: High-level exported parser functions (`parse_protocol3()`, `parse_protocol3_abx()`, and `parse_protocol3_inhibition()`).
...
- `parse_protocol3_inhibition()`: Reads a ProtoCOL 3 inhibition zone `.ods` report and returns a structured list of plates.
